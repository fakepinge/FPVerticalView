...
- (void)contentViewAddSubView:(UIView *)view {
    ...
    view.hiddenObserver = [APPObserver observerForObject:view keyPath:@"hidden" oldAndNewBlock:^(id  _Nonnull object, id  _Nonnull oldValue, id  _Nonnull newValue) {
        [weakSelf refreshLayoutSubview];
    }];
    
    view.frameObserver = [APPObserver observerForObject:view keyPath:@"frame" oldAndNewBlock:^(id  _Nonnull object, id  _Nonnull oldValue, id  _Nonnull newValue) {
        UIView *view = (UIView *)object;
        view.initialWidth = view.frame.size.width;
        view.initialHeight = view.frame.size.height;
        [weakSelf refreshLayoutSubview];
    }];
    if (!view.isSpecialHasMargin) { // 单独设置了间距就是用自己的特定的间距
...
- (void)setVerMargin:(CGFloat)verMargin {
    ...
    [[self subVerticalViews] enumerateObjectsUsingBlock:^(UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (!obj.isSpecialHasMargin) { // 单独设置了间距就是用自己的特定的间距
